...
local TYPES = {
    ...
    F = {
        4,
        function (b, fchar) return bc.float_to_bytes(b[1], 'f', fchar) end,
        function (b, fchar, i)
    ...
    D = {
        8,
        function (b, fchar) return bc.float_to_bytes(b[1], 'd', fchar) end,
        function (b, fchar, i)
...
function module:new(bytes, order)
	bytes = bytes or { }

    if order == "LE" or order == '<' then
        order = '<'
    else
        order = '>'
    end
...
function module:pack(format, values)
    ...
    fchar = fchar == '!' and self.order or fchar
    local res = {}

    if table.has({'<', '>', '!'}, formats[1]) then
...
function module:put_bool(val)
    self:pack(self.order .. "?", {val})
end

function module:put_uint16(val)
    self:pack(self.order .. "H", {val})
end

function module:put_sint16(val)
    self:pack(self.order .. "h", {val})
end

function module:put_uint32(val)
    self:pack(self.order .. "I", {val})
end

function module:put_sint32(val)
    self:pack(self.order .. "i", {val})
end

function module:put_float32(val)
    self:pack(self.order .. "F", {val})
end

function module:put_float64(val)
    self:pack(self.order .. "D", {val})
end
